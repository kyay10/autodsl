...
class ParameterFactory<A, T : A, C : A, P : A>(resolver: SourceInfoResolver<A, T, C, P>) :
    ...
    fun getParameters(constructor: C): List<Parameter> = constructor.getParameters().mapIndexed { index, parameter ->
        ...
        val (hasNestedDsl, collectionType) = when (rawType) {
            ...
            list, collection, iterable -> parameter.hasAnnotatedTypeArgument(AutoDsl::class) to ListType(
                findSingular(parameter, index)
            )
    ...
    private fun findSingular(parameter: P, index: Int): String = when {
        parameter.hasAnnotation(AutoDslSingular::class) -> parameter.getAnnotationProperty(
            AutoDslSingular::class,
            AutoDslSingular::value
        )
