...
class DslGenerator<A, T : A, C : A, P : A>(
    ...
    fun process(): List<T> = getClassesWithAnnotation(AutoDsl::class).flatMap {
        processClass(it, it.getAnnotationTypeProperty(AutoDsl::class, AutoDsl::dslMarker))
    }

    private fun processClass(clazz: T, markerType: ClassName?): List<T> {
        when (clazz.getClassKind()) {
            ...
            ClassKind.ANNOTATION_CLASS -> getClassesWithAnnotation(clazz)
                .flatMap { processClass(it, markerType) }
                .ifNotEmpty { return this + clazz }

            ClassKind.CLASS -> if (!generate(clazz, markerType)) return listOf(clazz)
        }
        return emptyList()
    }
     ...
     */
    private fun generate(clazz: T, markerType: ClassName?): Boolean {
        if (clazz.isAbstract()) {
                    ...
                    parameters.filter { it.isMandatory }.groupBy { it.group }.forEach { (_, parameters) ->
                        addStatement(
                            ...
                            parameters.joinToString(
                                separator = ",·",
                                prefix = if (parameters.size > 1) "One·of·" else ""
                            ) { it.name }
                        )
                    }
                    if (parameters.any { it.hasDefault }) {
    ...
    private fun TypeSpecBuilder.addParameterBuilderStyleVarargSetter(
        ...
        addFunction("with${parameter.name.replaceFirstChar { it.uppercase() }}") {
            val elementType =
                (parameter.typeName as ParameterizedTypeName).typeArguments.first()
